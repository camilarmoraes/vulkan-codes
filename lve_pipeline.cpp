...
namespace lve {
    ...
    void LvePipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo){
        ...
        assert(
            ...
            "Não foi possível criar um Pipeline Gráfico : Não foi providenciado o pipelineLayout no configInfo");

        assert(
            configInfo.renderPass != VK_NULL_HANDLE &&
            "Não foi possível criar um Pipeline Gráfico : Não foi providenciado o renderPass no configInfo");

        auto vertCode = readFile(vertFilepath);
        auto fragCode = readFile(fragFilepath);

        // std::cout << "Tamanho do codigo do Vertex Shader " << vertCode.size() << "\n";
        // std::cout << "Tamanho do codigo do Fragment Shader " << fragCode.size() << "\n";
        ...
        if (vkCreateGraphicsPipelines (lveDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo,nullptr, &graphicsPipeline) != VK_SUCCESS){
            throw std::runtime_error("Falha na criação de Graphic Pipeline");
        }
    ...
    void LvePipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule){
        ...
        if (vkCreateShaderModule(lveDevice.device(), &createInfo, nullptr, shaderModule) != VK_SUCCESS){
            throw std::runtime_error("Falha na criação de um Shader Module");
        }
